...
class QazzianDB{
	 ...
	 * @var array
	 */
	var $tables = array('users', 'usermeta', 'paths', 'links', 'link2cat');
	var $objects = array('Content', 'Config', 'Menu', 'MenuItem');
	...
	function __construct($dbuser, $dbpassword, $dbname, $dbhost) {
		...
		if ( defined('DB_DEBUG') and DB_DEBUG == true )
			$this->show_errors();
...
"/*/WP_I18N_DB_CONN_ERROR*/, $dbhost));
	...
	*/
	function set_prefix($prefix) {
		...
		if ( preg_match('|[^a-z0-9_]|i', $prefix) )
			return new DB_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Invalid database prefix'/*/WP_I18N_DB_BAD_PREFIX*/);
	...
	*/
	function bail($message) {
		if ( !$this->show_errors ) {
			if ( class_exists('DB_Error') )
				$this->error = new DB_Error('500', $message);
			else
				$this->error = $message;
			return false;
		}
	...
	*/
	function check_database_version()
	{
		...
		if ( version_compare($this->db_version(), '4.0.0', '<') )
			return new DB_Error('database_version',sprintf(__('<strong>ERROR</strong>: WordPress %s requires MySQL 4.0.0 or higher'), $wp_version));
	}
	...
	*/
	...
	static function instantiate($site_config) {
		return new QazzianDB(
			$site_config->db_user,
			$site_config->db_password,
			$site_config->db_name,
			$site_config->db_host
		);
	}
	...
	/**
	 * Returns the table schema required for all defined Objects.
	 */
	function getFullSchema() {
		$schema = array();
		foreach ( $this->objects as $obj ) {
			$schema[] = $obj::Get_Schema()->toString();
		}
		return $schema;
	}
		 ...
		 * @since 0.71
		 */
#		$wpdb = new QazzianDB(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST);
#}
?>
